...
class VariableFeatureCombiner:
    ...
    def load_ufo_features(self):
        """Load feature.fea content from all UFO masters (checks for expanded versions first)"""
        for source in self.designspace.sources:
            if source.path:
                ...
                if not os.path.isabs(ufo_path):
                    ...
                    ufo_path = os.path.normpath(os.path.join(designspace_dir, ufo_path))
            ...
            try:
                # Check for expanded features first in the UFO directory
                expanded_path = None
                for name in ['features_expanded_mark.fea', 'features_expanded.fea']:
                    p1 = os.path.join(ufo_path, 'features', name)
                    p2 = os.path.join(ufo_path, name)
                ...
                if expanded_path:
                    ...
                    print(f"Loaded expanded features from {expanded_path}")
                else:
    ...
    def extract_positioning_data(self, features_text, master_key):
        ...
        # Extract kern positioning (supports optional backslash and typical glyph name characters)
    ...
    def extract_mark_anchors(self, features_text, master_key):
        """Extract mark anchor data from lookup blocks with optional anchor coords fallback to UFO"""
        location = self.masters_data[master_key]['location']
        ...
        for lookup_match in re.finditer(lookup_pattern, features_text, re.DOTALL):
            ...
            # Extract markClass statements (making anchor coordinates optional in features)
            ...
            for mark_match in re.finditer(markclass_pattern, lookup_content):
                ...
                x_str = mark_match.group(2).strip()
                y_str = mark_match.group(3).strip()
                mark_class = mark_match.group(4)
                
                font = self.masters_data[master_key]['font']
                x_coord = None
                y_coord = None
                ...
                if x_str and y_str:
                    ...
                    # Fallback to UFO glyph anchors
                    anchor_name = f"_{mark_class}"
                    glyph_names = [g.replace('\\', '').strip() for g in glyphs.split() if g.strip()]
                    if glyph_names:
            ...
            # Extract pos base statements (making anchor coordinates optional in features)
            ...
            for base_match in re.finditer(posbase_pattern, lookup_content):
                ...
                x_str = base_match.group(2).strip()
                y_str = base_match.group(3).strip()
